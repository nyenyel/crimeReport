...
export default function UserOutlet() {
  ...
  const getUsers = async() =>{
    setLoading(true)
    try{
      const response = await apiClient.get(`v1/crud/user`)
                      ...
                      {data?.map((item, index)=> (
                        <tr className="bg-white border-b" key={index}>
                            ...
                            <td className="px-6 py-4 text-right">
                                {item?.isVerified
                                  ?
                                  <NavLink to={`profile/${item?.id}`} className="font-medium text-white px-4 py-2 bg-prc rounded-md mr-4 hover:bg-opacity-85">Edit</NavLink>
                                  :
                                  <NavLink to={`verify/${item?.id}`} className="font-medium text-white px-4 py-2 bg-src rounded-md mr-4 hover:bg-opacity-85">Verify</NavLink>
                                }
                            </td>
                        </tr>
                      ))}
